...
def CreateFileSetuplog(NameFile):
	...
	f = open(NameFile,"w")
	f.write('"IGP-ROJ Magnetometer logging setup file Ver 11.21"\n')
	...
	f.write('"cte1 Pendiente           ",.305185095\n')
...
def FechaHora(DataDic,StringTime):

	DataHour = int(''.join(StringTime[:2]))
	DataMinute = int(''.join(StringTime[2:4]))
	DataSecond = int(''.join(StringTime[4:6]))
	DataDay = int(''.join(StringTime[6:8]))
	DataMon = int(''.join(StringTime[8:10]))
	DataYea = int(''.join(StringTime[10:]))
...
def CreateFileMinM(DicComp, DataDic,string_DateMin, NameFile, NumDay):
	...
	if len(DicComp) != 0:
		...
		for key in DicComp:
			...
			DicChannelmV[key][0] = '%+09.2f'%ValInt
		...
		DicChannelmV['X'][0] = '%+09.2f'%ValXCorr
		DicChannelmV['Z'][0] = '%+09.2f'%ValZCorr
...
def CreateFileSec(DicComp, DataDic,string_DateSec, NameFile, NumDay):
	...
	if len(DicComp) != 0:
		...
		fileSec.write('%+08d '%DicComp['X'] + '%+08d '%DicComp['Y'] + '%+08d '%DicComp['Z'] + '%+08d '%DicComp['T1'] + '%+08d\n'%DicComp['T2'])
		fileSec.close()
